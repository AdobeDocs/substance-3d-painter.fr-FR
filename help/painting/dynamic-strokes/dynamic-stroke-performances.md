...
Pour les Traits dynamiques, les performances du graphe de Substance importent beaucoup car la Substance peut être régénérée de nombreuses fois en très peu de temps. Si un calcul de Substance est trop lourd, il peut créer une latence et par conséquent saccager et se figer lorsque vous peignez. Tout cela peut finir par créer une mauvaise expérience de peinture. Cette page regroupe des informations et des recommandations sur l&#39;utilisation de la fonction de contour dynamique.
...
* **Consommation de mémoire** : la génération d&#39;un grand nombre de variations pour un graphe de Substance de données pourrait finir par consommer beaucoup de mémoire (car ces générations sont volatiles car elles sont créées à la volée).

## Utilisation des Variations et des paramètres d’Espacement
...
* **Espacement** : plus l’espacement est petit, plus le nombre de tampons créés lors de la peinture d’un contour est élevé. Parfois, il n’est pas nécessaire d’appliquer un coup de pinceau continu et l’utilisation d’un grand espacement peut également aider à mieux voir l’alpha/le matériau utilisé.
...
Le temps peut être utile pour créer des aspects très spécifiques, mais il s’agit en fait de l’une des variables les plus coûteuses à utiliser dans un graphe de Substance. La raison en est qu’il est très difficile d’obtenir des valeurs similaires d’un coup de pinceau à l’autre. Le moteur du pinceau générera donc probablement de nouvelles variations en permanence. Si possible, évitez-le et utilisez plutôt l’espacement et l’index de tampon, qui, une fois combinés, peuvent donner des résultats similaires.

## Utilisation de StampIndex et de StampCycleCount

L&#39;**StampIndex** est l&#39;ID d&#39;un tampon individuel dans un coup de pinceau. Par défaut, il commence à 0 et augmente de 1 pour chaque nouveau tampon. Le **StampCycleCount** permet de limiter la quantité d&#39;index uniques et indique à Substance 3D Painter de recycler/réutiliser les graphes de Substance déjà générés. Lorsque l’ID actuel atteint la limite, Substance 3D Painter recommence à partir de 0, en créant une boucle.
...
* **StampIndex as RandomSeed** : lors de la création d&#39;un graphe de Substance, il est possible de définir le générateur aléatoire sur Absolu. Vous pouvez ainsi lui attribuer une valeur personnalisée qui peut être l’index de tampon. Cette opération crée une version de graphe de Substance unique pour chaque tampon à l’intérieur de votre trait.
* **Combiné avec StampCycleCount** : vous pouvez créer un ensemble limité de nouvelles variations, puis les réutiliser.
* **Démarrage aléatoire** : si le nombre de cycles est défini pour démarrer à partir d&#39;une valeur aléatoire au lieu de 0, cela signifie qu&#39;il utilisera obtenir une version de Substance différente au début pour chaque trait à l&#39;intérieur du pool de graphes déjà générés.

## Désactivation du calcul en fonction des valeurs des paramètres

Substance 3D Painter ne peut pas déterminer lors de l’ajustement d’un paramètre qu’il peut produire la même sortie, simplement parce que le calcul est masqué dans le graphe de Substance. C&#39;est en gros une boîte noire.

Pour améliorer les performances lors de l’ajustement des paramètres et de la peinture avec des Traits dynamiques, il est possible de spécifier à quel moment de nouvelles instances de graphe doivent être générées en utilisant des valeurs conditionnelles dans les champs userdata du graphe de Substance.
...
Dans ce cas, la valeur de départ aléatoire ne sera modifiée que si le paramètre de graphe (identifiant) **rondeur\_variation** est supérieur à 0 ou si les valeurs booléennes **flip\_x\_variation** ou **flip\_y\_variation** sont activées. Si la condition n’est pas remplie, le graphe ne sera pas régénéré. Les paramètres de graphe doivent être préfixés par «  **input.**   » à reconnaître.
