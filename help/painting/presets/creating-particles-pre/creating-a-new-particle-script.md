...
breadcrumb-title: ''
description: Apprenez à créer un script de particule dans Substance 3D Painter pour définir un comportement et des effets de forme de particule personnalisés.
...
* Un système de particule « \_Émetteur » qui est le modèle de base d&#39;un Émetteur.
* Système de particule « \_Receiver » qui est le modèle de base d&#39;un récepteur.
* Maillage sphère utilisé comme toile de fond par défaut de la scène

« \_Émetteur » et « \_Receiver » sont déjà prêts pour Painter. Ils ont déjà été configurés avec les évolutions, les champs, les décors nécessaires etc...

## Importation du maillage

PopcornFX ne prend en charge que **FBX**. Veillez à exporter votre maillage dans ce format. Lors de l’étape d’exportation, vérifiez la taille de votre maillage pour tenter de l’adapter aux unités correctes dans le « monde réel ».
...
## Modification de l’Émetteur/du destinataire

Nous dupliquerons les systèmes de particule existants et les adapterons pour tenir compte du nouveau maillage.

Cliquez avec le bouton droit de la souris sur le système de particule de données « \_Émetteur » (dans le dossier « Particules »), puis sélectionnez « Clone » (ou « Dupliquer ») pour créer votre propre Émetteur.

Ouvrez-le et, dans la fenêtre « Arborescence des Particules » (en bas à gauche), sélectionnez « **Calque\_Model** » qui doit se trouver dans : « Propriétés de l’éditeur => Toile de fond => Calques 3D ».
...
Ouvrez-le et, comme pour l&#39;émetteur, remplacez le mannequin par votre maillage dans « Layer\_Model ». Nous avons **modifié le maillage** **affiché à l&#39;écran** , mais nous devons également modifier **le maillage** **utilisé par les particules** .

Pour ce faire, dans la fenêtre « Arborescence des Particules », cliquez sur « **Forme** » qui doit se trouver dans : « Effet de Particule => Générateur => Calque\_1 => Échantillonneurs => Maillage ».
...
Ouvrez votre émetteur (celui que nous avons dupliqué précédemment) et dans la fenêtre « Arborescence des Particules », cliquez sur « Événements » qui doit se trouver dans : « Effet Particule => Créateur ». Remplacez ensuite le récepteur par votre récepteur en cliquant sur « Extern ».\
...
Ouvrez votre récepteur, et dans la fenêtre « Arborescence des Particules », sélectionnez «  CParticleEvolver\_Script «  (le premier qui vous est dédié :)) qui doit se trouver dans : « Effet de Particule => Calque\_1 => Etat\_0 ».
...
Pour les utiliser, vous devez d&#39;abord cliquer sur le bouton « Basculer les particules ».
