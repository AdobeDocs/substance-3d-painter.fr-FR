...
Les calques peuvent être masqués afin d’afficher/d’appliquer leur contenu uniquement sur des parties spécifiques de la texture. Le masque fonctionne comme un paramètre d’intensité sur le contenu du calque. Un masque sur un calque est toujours en niveaux de gris, quel que soit le contenu utilisé pour effectuer un peinture dessus (par conséquent, toute couleur sera convertie en niveaux de gris avant d’être peinte).
...
* Vous pouvez visualiser le masque lui-même en **Alt + clic gauche** sur sa vignette. Le viewport passe alors à une vue isolée du masque à partir de ce calque. Cette opération est également disponible via les paramètres de la visionneuse.
...
Il est possible de créer immédiatement un masque lors de la création d&#39;un calque de remplissage (par glisser-déposer) si la touche **CTRL** est enfoncée :
...
La ligne sous chaque vignette d’un calque indique s’il existe des effets. Le gris n’entraîne aucun effet, le rouge au moins un effet. Il existe une pile d’effet par masque et par contenu.
...
Les masques adaptables permettent d’enregistrer un masque et son effet afin de les réutiliser facilement sur d’autres calques ou projets. Pour créer un masque adaptable, faites un clic droit dessus et choisissez « **Créer un masque adaptable** ».\
Lorsque vous faites glisser et déposez un masque adaptable sur un calque, un masque noir est créé s’il n’existe pas déjà, sinon la liste d’effets est fusionnée avec la liste existante. Il est possible d&#39;écraser complètement la liste d&#39;effets en maintenant la touche « **CTRL** » enfoncée lors de la suppression du masque adaptable.
