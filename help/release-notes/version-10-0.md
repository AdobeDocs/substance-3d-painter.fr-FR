...
Cette nouvelle version introduit la <b>ressource Texte</b>, qui permet de charger des fichiers de polices pour écrire du texte dans différents contextes (pinceaux, projections de remplissage, entrées d&#39;image de Substance, etc.) pour embellir vos textures.
...
* <b>Prise en charge des fichiers Illustrator (.Ai)</b>\
  Dans cette nouvelle version, les fichiers .ai peuvent désormais être importés et rendus dans Painter pour être utilisés comme ressources dans les pinceaux, les projections de remplissage ou comme entrées d’image de Substance.
...
Cette version comprend l’ajout du nouveau module de pile de calques dans notre API Python. Cette API permet de contrôler la pile de calques d’un projet, ouvrant ainsi la voie à la création de plug-ins de pile de calques avancés et d’outils personnalisés.
...
* <b>Gestion de base des couleurs</b>\
  Avec l’exposition de la pile de calques, nous avons dû introduire la notion de gestion des couleurs dans notre API. Un nouveau module de <b>gestion des couleurs</b> a été ajouté pour créer, ajuster les couleurs et choisir l&#39;espace colorimétrique des bitmaps. (Cette partie de l’API n’est pas encore terminée et sera développée dans les versions futures.)
* <b>Informations sur le paramètre prédéfini d&#39;exportation de requête</b>\
  Les paramètres prédéfinis d’exportation sont désormais exposés dans notre API, ce qui permet d’interroger la liste des paramètres prédéfinis (à la fois prédéfinis et personnalisés). Leur contenu peut également être récupéré dans un format similaire à notre API de textures d’exportation existante.
...
>[!NOTE]
>
> Vous trouverez également des exemples de plug-ins de pile de calques dans notre [documentation en ligne](https://adobedocs.github.io/painter-python-api/).

### Amélioration de la peinture à la map normal

![](../assets/v10_banner_flow-1.jpg)

Dans cette version, nous avons retravaillé le workflow de peinture de map normal. Nous avons notamment modifié la façon dont nous accumulons et mélangeons les tampons de pinceaux normaux. Ces modifications ont été apportées pour résoudre les problèmes liés à la peinture de cartes de flux.
...
* <b>Transparence sur alpha zéro</b>\
  Les tampons de pinceau réalisés avec une texture avec un alpha à zéro dessinent désormais en transparence. L’exemple ci-dessous présente un tampon de pinceau (à gauche) et une projection planaire (à droite).
...
### Amélioration des mots de manipulateur de transforme

![](../assets/v10_banner_transform_v2-1.jpg)

Plusieurs améliorations ont été apportées pour améliorer l’utilisation des manipulateurs de transforme.

* <b>Mode Précision avec CTRL</b>\
  Appuyer sur le contrôle tout en faisant glisser sur un manipulateur va maintenant entrer dans un nouveau mode de précision qui permet des opérations plus méticuleuses. Cette modification s’applique aux manipulateurs translater, faire pivoter et mettre à l’échelle.\
...
* <b>Nouveau comportement d&#39;échelle</b>\
  L’intensité de l’échelle est désormais basée sur la valeur d’échelle actuelle et non plus sur la taille de la scène. Cela facilite les changements relatifs, en particulier pour les valeurs faibles. Associée au mode précis, elle rend la mise à l’échelle beaucoup plus agréable.\
...
* <b>Amélioration de la rotation du manipulateur de surface</b>\
  Le manipulateur de décalcomanie de surface est désormais beaucoup plus stable lorsqu&#39;il glisse autour d&#39;une surface. Il n&#39;augmente pas sa rotation simplement en faisant des allers-retours.\
...
* <b>projection alignée sur la Caméra lors du glisser-déposer</b>\
  Glisser-déposer une ressource dans le viewport permet de créer une projection de déformation directement sur la surface du maillage. Cette projection a été incorrectement tournée précédemment. Elle est maintenant alignée sur la caméra.
