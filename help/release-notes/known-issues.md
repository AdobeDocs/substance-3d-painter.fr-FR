...
* `[Baking]` seams Uv n&#39;apparaissent pas après la réimportation de mes
* `[Baking]` artefacts de type Grille avec certains paramètres
* L&#39;Ambient occlusion `[Baking]` Ignorer la face arrière par nom de maillage ne fonctionne pas
* `[Baking]` `[AMD]` Périphérique perdu lors du baking avec un lourd maillage en poly élevé
...
* `[Engine]` Erreur lors de l&#39;utilisation des Matériaux adaptables si le Jeu de textures n&#39;a pas de vignette 1001
* `[Engine]` Peindre avec l&#39;outil Clone dans des couleurs de décalage de couche normales de manière incorrecte
* Le masque de géométrie `[Engine]` affiche des artefacts aux bordures de l&#39;UV avec des calques instanciés
...
* Le paramètre « caméra_vp_matrix_inverse » `[Shader]` n&#39;est pas reconnu
...
* `[gltf]` Impossible d&#39;ouvrir les fichiers exportés via Babylon Exporteur
* `[Displacement]` Problème lors de la peinture
* `[Polygon Fill Tool]` Sélection incorrecte avec la symétrie
* Les traits `[2D view]` n’apparaissent pas toujours lorsque vous peignez
* `[Console]` symboles associés à raccourci ne peuvent pas être écrits
* Le message d&#39;erreur `[LOG]` est incorrect lors de l&#39;échec de l&#39;exportation
* Le Pochoir `[3D View]` ne fonctionne pas sur les objets dupliqués
* `[Resource updater]` Les différentes ressources de l&#39;étagère portant le même nom sont lues comme une seule ressource
* `[Sample]` caméra rompue dans l’exemple d’aperçu
* `[Instancing]` `[Projection]` Lors de la sélection d&#39;une instance dans proj planaire, un autre proj planaire est sélectionné sur un autre jeu de textures
...
* `[Mesh export]` Ne pas prendre en compte les nouveaux noms de jeu de textures
* `[Anchor Points]` couleur incorrecte lors de l&#39;utilisation dans le générateur
* Le baker Map id `[Bakers]` ne prend pas en compte le Matériau physique 3ds Max 2021
* `[UV Tiles]` Aucun message d&#39;erreur lors du chevauchement d&#39;espaces d&#39;UV avec un maillage spécifique
* `[GLTF]` `[Crash]` La création d&#39;un projet avec un fichier gltf compressé provoque un crash
* Les mappages de position `[UV Tile sequence]` ne sont pas importés correctement
* Le masque de combinaison d&#39;Heights `[UVTiles]` n&#39;est pas actualisé avec le masque de Tuile UV
...
* `[Layer stack]` Copier dans le masque basculer vers le mode de matériau
* Faute de frappe `[UI]` dans les paramètres du créateur de pinceaux
* `[Texture Set Settings]` Nom d&#39;instance de shader incorrect après un renommage
...
* `[Geometry mask]` Problème lors de la réimportation du maillage et du calque d’instance
* Espace colorimétrique `[Color management]` introuvable lorsque la mosaïque 1001 est manquante
* displacement `[Export mesh]` non exporté avec des UV définis
* `[RedHat]` problèmes de sélecteur de couleurs
* Le menu contextuel `[Regression]` `[UI]` est trop petit sur l&#39;écran HD
* `[Resources]` maps de maillage importées sont ignorées par la mise à jour automatique
...
* Les projets comportant un seul jeu de textures sont rouverts en mode Base color solo
...
* Le comportement du bouton de restauration n’interagit pas avec la clé de verrouillage dans les paramètres de caméra
...
* Le pochoir n’est pas mis à jour lors de la modification du paramètre prédéfini de pinceau
...
* `[Crash]` Cliquer sur la liste de Jeux de textures après la création du projet qui a échoué provoque un crash
* `[Crash]` crash d&#39;erreur critique lorsque le même projet est ouvert deux fois
...
* `[Crash]` La création de textes très longs dans le crash de la boîte à ruban
* `[Crash]` Retour au mode peinture après la perte de l&#39;appareil lors du baking
* `[Crash]` Quitter Painter après avoir annulé l&#39;exportation des mappages
* `[Crash]` Exportation du maillage avec des symboles spéciaux dans le nom de la caméra
